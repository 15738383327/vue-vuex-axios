...
function requestUrl(url) {
  ...
  if (url.startsWith('locationAPI')) {
    url = url.replace('locationAPI', config.build.locationAPI)
  }
  return url
}
...
export default {
  ...
  delete(url, id, options, { load = true, loadMsg = '加载中...' } = {}) {
    return send(url + '/' + id, 'delete', null, null, options, load, loadMsg)
  }
}
